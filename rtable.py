...
'''
rtable : the buckets of a DHT node, it record the k-closet nodes.
htable : the record the id(sha1),which is the resource id, of node.
'''
...
class RoutingTable(object):
    ...
    def node_by_id(self, node_id):
        """ Get appropriate node by node_id """
        with self.lock:
            if node_id in self.nodes:
